...
def super_res_self_sim(img, s= 3.0):
    n,c= 1,0
    while n < s:
        n *= 1.25
        c += 1
    ...
    img_sr= img.copy()
    for i in range(c):
        img_sr= small_SR(img_sr, 1.25)
    final_size= (math.ceil(img.shape[0]*s), math.ceil(img.shape[1]*s))
    img_sr= resize(img_sr, final_size, interp='cubic')
    return img_sr
